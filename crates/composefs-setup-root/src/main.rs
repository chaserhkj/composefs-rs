...
// Try parse cmdline with sha512 digest address first, if failed, parse again with legacy sha256 digest address
fn parse_image_address(cmdline: &str) -> Result<(String, bool)> {
    if let Ok((id, insecure)) = get_cmdline_composefs::<Sha512HashValue>(cmdline) {
